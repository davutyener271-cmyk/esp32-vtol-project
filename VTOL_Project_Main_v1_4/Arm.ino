void update_Arm() {
  ...
  if (iBusChannels[4] > 1800 && armSwitchWasOff) {
    ...
    if (iBusChannels[2] <= 1050) {
      ...
      // ARM anindaki son gecerli basinci sifir metre kabul et.
      if (reset_BMP_Altitude_Reference()) {
        Serial.printf(
          "ARM: irtifa sifirlandi. Referans: %.2f hPa\n",
          bmpReferencePressure / 100.0f
        );
      } else {
        Serial.println(
          "ARM: BMP referansi yenilenemedi; onceki referans kullaniliyor."
        );
      }
    }
  }
